...
def main():
    out_full_text = open(tweets_file + ".full", 'w', encoding="utf8")
    out_text = open(tweets_file + ".text", 'w', encoding="utf8")
    out_labels = open(tweets_file + ".labels", 'w', encoding="utf8")
    out_ids = open(tweets_file + ".ids", 'w', encoding="utf8")
    tot = 0
    ok = 0
    with open(tweets_file, encoding="utf8") as f_in:
        for line in f_in:
            j = json.loads(line)
            tweet_id = j['id']
            text = j['text'].replace("\n", "")
            out_full_text.write(text + "\n")
